...
void showMostPopularStops() {
    ...
    for (const auto &[stopId, count]: stopUsageCount) {
        ...
        if (it != stops.end()) {
            std::cout << "Stop: " << it->name << " | Times Used:  \n";
        }
    }
}
...
void showMostPopularTransport() {
    ...
    for (const auto &[transportId, count]: transportUsageCount) {
        ...
        if (it != transports.end()) {
            std::cout << "Transport: " << it->name << " | Used in Routes: \n";
        }
    }
}
...
void showReports() {
    ...
    switch (choice) {
        case 1: showMostPopularStops();
            break;
        case 2: showMostPopularTransport();
            break;
        case 3: showRouteStatistics();
            break;
        case 4: showStopsWithoutRoutes();
            break;
        case 5: showTopRoutesByStops();
            break;
        case 0: return;
